...
public class RackupApplication extends AbstractRackApplication {
    ...
    public RackupApplication(Ruby runtime, RackGrizzlyAdapter adapter) {
        super(createApplicationObject(runtime, adapter), adapter);
    }

    private static IRubyObject createApplicationObject(Ruby runtime, RackGrizzlyAdapter adapter) {
        runtime.evalScriptlet("require 'jruby/rack/rackup'");
        runtime.evalScriptlet("require 'rack/handler/grizzly'");

        IRubyObject app;
        if (System.getProperties().get("glassfish.rackupApp") != null) {
            app = (IRubyObject) System.getProperties().get("glassfish.rackupApp");
        } else {
            File configScript = adapter.config.framework().initScript();
            String path;
